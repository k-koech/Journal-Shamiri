...
{
  const { createJournal, updateJournal, isUpdate, setIsUpdate } = useJournalContext();
  const navigation = useNavigation<StackNavigationProp<RootStackParamList>>();

  const journal_route = route.params?.journal;
  const [journal, setJournal] = useState<any>(journal_route);  
  ...
  if(isUpdate){
    console.log('journal on Update', journal);
  }
  else{
       console.log('journal on the ADD', journal);
  ...
  useEffect(() => {
    if (journal) {
      const { title, content, category } = journal;
